function plotclick_GUI(XVector,YVector,CVector,useMZdata,MZvector,partData,timeData,sizeData,clustRelation) 
% PLOTCLICK sets up a gscatter figure of particles in FATES and allows the
...
% Call as plotclick_GUI(XVector,YVector,CVector,useMZdata,MZvector,partData,timeData,sizeData,clustRelation) 
%
% INPUT
...
%% callback functions
    ...
    function outputData(source,callbackdata)
        ...
        if findSelect <= clickcount %for single cluster selection
            ...
            outPID{2,1} = trackX(findSelect,:);
            outPID{3,1} = trackY(findSelect,:);
        else %to output all clusters
            outPID = cell(3,clickcount);
            for i = 1:clickcount
