...
for m = 1:length(uniqueCluster)
    ...
    averageClust(m,:) = mean(useMZdata(tmp,:),1);
...
%%
    ...
    function separateClusters(source,callbackdata)
        ...
        if any(tmp) && sum(tmp) ~= sum1 %make user input creates a split
            ...
            outputLabel = uniqueLabel;
            outputLabel{length(uniqueLabel)+1} = 'ALL';
            averageClust(clusterSelect3.Value,:) = mean(useMZdata(clustData == uniqueCluster(clusterSelect3.Value),:),1);
            averageClust(length(uniqueCluster),:) = mean(useMZdata(clustData == newC,:),1);
    ...
    function resetClusters(source,callbackdata)
        ...
        for q = 1:length(uniqueCluster)
            ...
            averageClust(q,:) = mean(useMZdata(tmp,:),1);
        end
    ...
    function combineClusters(source,callbackdata)
        ...
        if cluster1 ~= cluster2 %make sure different clusters are selected
            ...
            clusterColor(tmp,:) = repmat(colorLabel(minLabel,:),sum(tmp),1); %fix color of particles to match min cluster
            averageClust(minCIDX,:) = mean(useMZdata(clustData == minLabel,:),1); %recalc average cluster spectrum
